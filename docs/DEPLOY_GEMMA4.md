...
Cloudflare's free allocation is 10,000 neurons/day. Gemma 4 is priced at roughly
9,091 neurons per million input tokens and 27,273 per million output tokens, so
a representative RAG request (4,000 input + 500 output) costs about **50
neurons** — roughly **200 answers/day**.

`LLM_DAILY_BUDGET=180` trips below that, so students get a sentence they can act
on rather than an opaque provider error mid-stream. Treat 200 as a pilot
estimate, not a guarantee: follow-up condensation, long history and longer
answers all cost more. `/admin/usage` shows the live count.
...
| Quota exhausted early | lower `LLM_DAILY_BUDGET`; students get `LLM_BUDGET_EXHAUSTED` |
